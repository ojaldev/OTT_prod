...
interface GenrePlatformHeatmapProps {
...
const GenrePlatformHeatmap: React.FC<GenrePlatformHeatmapProps> = ({ filters, height = 400 }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const resp = await analyticsService.getGenrePlatformHeatmap(filters);
        const raw: any = (resp as any)?.data ?? resp;
        const arr: HeatmapItem[] = Array.isArray(raw)
          ? raw
  ...
  const { genres, platforms, matrix, min, max } = useMemo(() => {
    const gSet = new Set<string>();
    const pSet = new Set<string>();
    items.forEach(i => { gSet.add(i.genre); pSet.add(i.platform); });

    const platforms = Array.from(pSet).sort((a, b) => a.localeCompare(b));

    // Sort genres by total count desc for better readability
    const totals = new Map<string, number>();
    items.forEach(i => totals.set(i.genre, (totals.get(i.genre) || 0) + i.count));
    const genres = Array.from(gSet).sort((a, b) => (totals.get(b) || 0) - (totals.get(a) || 0));

    const map = new Map<string, number>();
    items.forEach(i => map.set(`${i.genre}__${i.platform}`, i.count));

    const matrix: number[][] = genres.map(g => platforms.map(p => map.get(`${g}__${p}`) || 0));
    ...
    return { genres, platforms, matrix, min, max };
  }, [items]);

  const ratio = (v: number) => {
    if (max === min) return v > 0 ? 1 : 0; // avoid divide by zero
    return (v - min) / (max - min);
  };

  // Theme-aware purple scale for good contrast in both modes
  const h = 270; // purple hue
  const s = 70; // saturation
  const lStart = theme === 'dark' ? 22 : 96; // low values
  const lEnd = theme === 'dark' ? 56 : 44;   // high values
  ...
  const handleDownload = async () => {
    ...
    if (!el) return;
    // Expand scroller to capture full grid
    ...
    try {
      ...
      link.download = 'genre-platform-heatmap.png';
      link.href = dataUrl;
      link.click();
    } catch (e) {
      // ignore
  ...
  return (
    <>
      <div
        ...
        data-chart-title="Genre-Platform Heatmap"
      >
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h3 className={`text-lg font-semibold ${titleColor}`}>Genre-Platform Heatmap</h3>
            <p className={`text-sm ${secondary}`}>Counts of content across Genres (rows) and Platforms (columns)</p>
          </div>
          <ChartToolbar
        ...
        {showInfo && (
          <div className="mb-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
            <div className="flex justify-between items-start">
              <p className={`text-sm ${secondary}`}>Heatmap showing the distribution of content counts across genres and platforms. Darker cells indicate higher counts.</p>
...
export default GenrePlatformHeatmap;
